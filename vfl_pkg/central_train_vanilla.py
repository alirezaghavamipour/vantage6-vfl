...
@algorithm_client
def central_train_aggvflc_vanilla(
    ...
    compare against vantage6-vfl's real (Rep3 MPC) 'Train aggVFLc
    model' function. Do not use this for any data that needs real
    protection.

    Orchestrates a full plaintext vertical logistic regression training
    run: dispatches vanilla_train_worker_run to each feature party and
    vanilla_train_coordinator_run to the label party, who coordinate
    training directly with each other over a plain socket (not MPC).
    Row alignment still uses the same private Rep3 PSI circuit as every
    other function here (dispatched to agg_org_ids) - only the training
    math itself skips MPC.
    ...
    a well-documented Direct Label Inference leak in the VFL literature.
    The real aggVFLc function exists specifically to close this gap by
    keeping all of this inside a Rep3 MPC computation instead.

    Unlike 'Train aggVFLc model', this takes feature_org_ids and
    label_org_id separately (not one combined client_org_ids list) -
    the label party plays a structurally different role here (it's the
    plaintext coordinator every feature party connects to directly),
    where in the MPC version every client's role is symmetric from the
    ...
    info(f"Central (train aggVFLc VANILLA - NOT PRIVATE): starting training run "
    ...
    for org_id in feature_org_ids:
        t = client.task.create(
            input_={"method": "vanilla_train_worker_run", "kwargs": kwargs},
            organizations=[org_id],
            name=f"vanilla-train-worker-{org_id}",
        )
        tasks[org_id] = t["id"]

    t = client.task.create(
        input_={"method": "vanilla_train_coordinator_run", "kwargs": kwargs},
        organizations=[label_org_id],
        name=f"vanilla-train-coordinator-{label_org_id}",
    )
    ...
    for org_id in agg_org_ids:
        t = client.task.create(
            ...
            organizations=[org_id],
            name=f"vanilla-train-psi-agg-{org_id}",
        )
        tasks[org_id] = t["id"]

    info(f"Central (train aggVFLc VANILLA): all {len(tasks)} sub-tasks submitted, waiting for results...")
    ...
    info(f"Central (train aggVFLc VANILLA): training complete - aligned_count={aligned_count} "
